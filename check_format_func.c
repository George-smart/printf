...
int check_format_func(const char *format, va_list, func)
{
    ...
    for(i = 0; format && format[i] != 0; i++)
    {
        ...
        {
            // loop through the specifier stored in st_format array struct
            for(j = 0; st_format[j].fmt; j++) 
            {
                // check if the string character equals the specifier
                if (format[i + 1] == st_format[j].fmt[k])
                {
                    // call the st_format struct funct
                    fn = st_format[j].func(list); /* This line of code calls the functions in the struct array */
                    if (fn == -1)
                        return -1;
                    counter += fn;
                    i++;
